...
Thank you for improving `aws.modules.cognito`. This guide covers the toolchain, the local quality gate, how features are tested and where they belong, commit and pull request conventions, and how releases are cut.
...
make integration-smoke   # about a minute; one user pool with one client, created and deleted
```

Add a suite when a feature's correctness depends on the AWS API rather than on rendering (for example a new schema-attribute shape or a Lambda trigger path). A Cognito user pool needs no VPC or other prerequisite, so no fixture module exists today; keep every value derived from the environment and a random suffix, and never reference a real account beyond the credentials supplied.
...
| `make security` | `checkov -d . --framework terraform`, and `trivy config --severity HIGH,CRITICAL` when trivy is on the PATH. Checkov has no built-in policies for `aws_cognito_*` resources, so a passing count here reflects generic Terraform checks, not Cognito-specific coverage; Trivy does have Cognito checks (deletion protection, MFA) and must report zero findings. A skip needs an inline `checkov:skip=` comment with a reason on the resource it concerns; there are none today. |
...
- Tests live in `tests/cognito_userpool.tftest.hcl`, starting with `mock_provider "aws" {}` and a `variables` block holding a valid baseline; each `run` overrides only what it exercises.
- Use `command = plan`. Nothing here talks to AWS, so tests run in seconds and in CI without credentials.
- Validations are tested with `expect_failures`. Point it at the object that carries the check: `[var.advanced_security_mode]` for a variable validation, `[aws_cognito_user_pool_client.this]` for a precondition on a client. A run with `expect_failures` passes only if exactly those objects fail; add a positive run alongside so the happy path is covered too.
- Assertions must not depend on unknown values. With a mock provider, computed attributes such as ARNs and IDs are unknown at plan time. Assert on what the module knows by construction: rendered arguments, resource counts, tags, and outputs derived from inputs.
...
| Concern | Lives in |
| --- | --- |
| A user-pool argument (a new Cognito setting) | `variables.tf` with a description, type, and validation; `main.tf` to render it; a test covering the rendering and, if it is validated, an `expect_failures` run. |
| Tag computation, the Lambda-trigger map | `locals.tf`. |
| Outputs | `outputs.tf`; every output has a description, and one that is derived from inputs is asserted in a test. |

Rules that apply everywhere: no data sources (derive from inputs and the provider's region), every variable has a description, a type, and a validation where a wrong value would otherwise fail at apply time, every output has a description, defaults are the secure choice, MFA can never be configured off, and client secrets are never an output.
...
```text
feat(schema): validate developer_only_attribute on custom attributes
fix(main): strip null lambda_config entries before rendering
docs: explain the advanced-security cost trade-off
test(email): cover a non-SES source_arn rejection
...
2. Create a signed annotated tag on the merge commit. The signing key must be registered with GitHub so the tag shows as Verified:
   ...
   ```sh
   git tag -s vX.Y.Z -m "aws.modules.cognito vX.Y.Z"
